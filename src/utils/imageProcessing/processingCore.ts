...
import { compressImage, createObjectUrl } from '@/utils/imageUtils';
...
export async function processSingleImage(
  ...
  try {
    ...
    if (removeBackground) {
      const bgRemovalResult = await handleBackgroundRemoval(
        image, 
        apiKey, 
        selfHosted, 
        serverUrl, 
      ...
      if (bgRemovalResult.success) {
        processedFile = bgRemovalResult.file;
...
async function handleBackgroundRemoval(
  ...
  try {
    ...
    if (bgRemovalResult.processedFile) {
      ...
      return { success: true, file: bgRemovalResult.processedFile };
    } else {
      ...
      return { success: false, file: image.original };
    }
  } catch (error) {
    // All retries failed
...
async function handleCompression(
  ...
  try {
    ...
    const compressedFile = await retryOperation(
      () => compressImage(file, compressionOptions),
      {
        maxRetries: 2,
        delayMs: 800,
        onRetry: (attempt) => {
          console.log(`Retrying compression (attempt ${attempt}/2) for ${originalFileName}`);
          toast({
            title: "Retrying Compression",
            description: `Attempt ${attempt}/2 for ${originalFileName}`
          });
        }
      }
    );
    ...
    toast({
      ...
      description: `All retry attempts failed for ${originalFileName}`
    });
